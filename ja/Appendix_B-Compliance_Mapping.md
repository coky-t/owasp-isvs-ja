...
NIST Special Publication 800-213A "IoT Device Cybersecurity Requirements Catalog" は SP 800-213 ("IoT Device Cybersecurity Guidance for the Federal Government") を補完するものです。これは、連邦機関やその他の組織が IoT デバイスのサイバーセキュリティ要件を特定、評価、規定するために使用できる、IoT デバイスレベルの技術的機能と非技術的支援機能のカタログを提供します。SP 800-213A はデバイスのサイバーセキュリティ機能を七つの技術的領域 (デバイス識別 (Device Identification, DI)、デバイス設定 (Device Configuration, DC)、データ保護 (Data Protection, DP)、インタフェースへの論理アクセス (Logical Access to Interfaces, LA)、ソフトウェア更新 (Software Update, SU)、サイバーセキュリティ状態の把握 (Cybersecurity State Awareness, CS)、デバイスセキュリティ (Device Security, DS)) と、さらに、文書化 (Documentation)、脆弱性/サイバーセキュリティ情報の受領 (Vulnerability/cybersecurity information reception)、サイバーセキュリティイベント通知 (Cybersecurity event notification) をカバーする非技術的支援機能に分類しています。各領域は具体的なサブ機能 (DI (IMS), DP (CRY), LA (IFC) など) にさらに細分化されており、デバイスレベルのセキュリティ動作に直接マップしています。このマッピングではその形式で SP 800-213A サブ機能識別子を参照しています。
